...
temperature_cube = np.zeros([l,l,l])
...
'''
r_fil = 0 # Radius of the filament in AU
n_0_fil = 0 # Central filament number density
T_0_fil = 0 # Central prestellar core temperature in K

r_proto = 0 # Radius of the protostar in AU
...
for n in range(0,len(z)-1):
    for m in range(0,len(y)-1):
        for l in range(0,len(x)-1):
            # Apply truth statement to conditions describing filament to determine whether point is within the filament
            if (x[l]-centre[0])**2 <= r**2 and (y[m]-centre[1])**2 <= r_fil**2 and (z[n]-centre[2])**2 <= r_fil**2:

                # Determine whether point within the filament also lies within the protostar
                if (x[l]-centre[0])**2 + (y[m]-centre[1])**2 + (z[n]-centre[2])**2 <= r_proto**2:

                    # Determine the distance to a given point in the filament
                    # ((x[l]-centre[0])**2 + (y[m]-centre[1])**2 + (z[n]-centre[2])**2)**(0.5) is the distance from the centre of the filament to the pixel in question
                    r_point = ((x[l]-centre[0])**2 + (y[m]-centre[1])**2 + (z[n]-centre[2])**2)**(0.5)

                    # Determine the number density at a given point in the filament
                    n_fil = n_0_fil*(1./(1+(r_point/r_fil)**2))
                    filament_density = n_fil*muh2*mp*d2g

                    # Repeat for temperature
                    T_fil = T_0_fil*(1+((r_point/r_fil)**2))
                    filament_temperature = T_fil
