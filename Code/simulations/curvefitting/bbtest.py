...
loglog(wav,fluxdensity[0],'b--',label=(str(r'$\beta$=')+str(B[0])))
loglog(wav,fluxdensity[1],'g--',label=(str(r'$\beta$=')+str(B[1])))
grid(True,which="majorminor",ls="-",color='0.65')
title(str('Two SEDs for ')+str(r'$\beta=$')+str(B[0])+str(r' and $\beta=$')+str(B[1])+str(r' at fixed $N=$'+str(N)+str(r'$g cm^{-2}$ and  $T=20K$')))
xlabel('Wavelength $(cm)$')
ylabel('Flux Density $(ergs/cm^{2}/Hz)$')
legend(loc='best')
savefig('flux_density_combined.png',dpi=300)
close()
