...
# Find the index of the frequency that most closely matches the frequency of the band (i.e. find the difference between the two and find the index at which this is the minimum)
psw_index = min(enumerate(v), key=lambda x: abs(x[1]-psw[0,2]))
pmw_index = min(enumerate(v), key=lambda y: abs(y[1]-pmw[0,2]))
plw_index = min(enumerate(v), key=lambda z: abs(z[1]-plw[0,2]))
blue_index = min(enumerate(v), key=lambda a: abs(a[1]-blue[0,2]))
green_index = min(enumerate(v), key=lambda b: abs(b[1]-green[0,2]))
red_index = min(enumerate(v), key=lambda c: abs(c[1]-red[0,2]))
