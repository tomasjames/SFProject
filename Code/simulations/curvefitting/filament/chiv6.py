...
dust_density = np.loadtxt('../../filament/blue/core/dust_density.inp', skiprows=3)
dust_temperature = np.loadtxt('../../filament/blue/core/dust_temperature.dat', skiprows=3)
...
imag = radmc3dPy.image.readImage('../../filament/blue/core/image.out')
...
N = np.linspace(np.log10(10**18), np.log10(10**26), 40)
...
N = np.linspace(np.log10(min_N-min_N/2), np.log10(max_N+min_N/2), 40)
T = np.linspace(min_T-min_T/2, max_T+min_T/2, 40)
...
for h in range(0,imag.nx*imag.ny):
    ...
    # Plot the data
        ...
        legend(loc='best')
        title('The $\chi^{2}$ Minimised Best Fit SED for PACS and SPIRE Bands for the (0,0) Pixel\n')
        savefig('averages_fine.png',dpi=300)
        close()
