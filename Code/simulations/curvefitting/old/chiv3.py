...
N = np.linspace(10**19, 10**25, 10**3)
...
# Loop through column density
for i in range(0,len(N)):

    # Declare loop to keep values of blackbody at each value of N
    mod_loop = []
    ...
    # Loop through all T for a given N
    for j in range(0,len(T)):
        blackbody = mbb(N[i],dust_mass,opacity,v,T[j])

        # Append to list to store continued values
        mod.append(blackbody)
        mod_loop.append(blackbody)

    # Loop through the values of mod_loop and determine the chi-squared values
    for k in range(0,len(mod_loop)):
        # Find the flux at the index determined earlier
        to_fit_psw = mod_loop[k][psw_index[0]]
        to_fit_pmw = mod_loop[k][pmw_index[0]]
        to_fit_plw = mod_loop[k][plw_index[0]]
        ...
        chivals_loop.append(chi(to_fit,ps,sigma=[psw[2],pmw[2],plw[2]]))

    # Determine the best fit blackbody by finding the index of the minimised chi-squared routine
    chi_min_index = chivals_loop.index(min(chivals_loop))
    chi_min_blackbody = mod_loop[chi_min_index]

    # Append value
    store.append(chi_min_blackbody)

new_chivals, new_chivals_loop, new_store = [],[],[]

for l in range(0,len(store)):
    # Find the flux at the index determined earlier
    to_fit_psw = mod[l][psw_index[0]]
    to_fit_pmw = mod[l][pmw_index[0]]
    to_fit_plw = mod[l][plw_index[0]]
    ...
    # Loop through each value of the expected data (i.e. each value of the column density) and determine the chi squared value for each value
    new_chivals.append(chi(to_fit,ps,sigma=[psw[2],pmw[2],plw[2]]))
    new_chivals_loop.append(chi(to_fit,ps,sigma=[psw[2],pmw[2],plw[2]]))

# Determine the best fit blackbody by finding the index of the minimised chi-squared routine
chi_min_index = new_chivals_loop.index(min(new_chivals_loop))
...
plot(v,chi_min_blackbody,label='Best Fit')
xlabel(r'$\nu \/(Hz)$')
...
savefig('SPIRE_averages_v3.png',dpi=300)
close()
