...
if os.path.isfile('../../../curvefitting/filament/green_average_data.txt') == True:
    print 'Data storage file already exists; opening now\n'
    save_data = open('../../../curvefitting/filament/green_average_data.txt', 'a+')
    data_store = csv.writer(save_data, delimiter=' ')

else:
    print 'Data storage file does not already exist; writing now\n'
    save_data = open('../../../curvefitting/filament/green_average_data.txt', 'w+')
    data_store = csv.writer(save_data, delimiter=' ')
...
xlabel(r'$\nu(Hz)$')
ylabel('Transmission')
suptitle('Spectral Energy Distribution for PSW Band Synthetic Data\n')
legend(loc='best',prop={'size':9})
tight_layout()
