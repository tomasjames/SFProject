...
chi_data = np.loadtxt('../../curvefitting/filament/chi_fine.txt',skiprows=1)
...
h, b = np.histogram(np.log10(N_chi_inp), bins=np.linspace(np.log10(np.amin(N_chi_inp)), np.log10(np.amax(N_chi_inp)), 100), density=True)
#bar(b[:-1],h,width=(max(h)-min(h))/len(N_chi_inp))
bar(b[:-1],h/np.sum(h),width=0.021)
...
colorbar(label='$log_{10}\sigma_{N}\/(g\/cm^{-2})$')
xlabel('X (pixels)')
ylabel('Y (pixels)')
...
subplot2grid((6,7), (5,0), colspan=4,rowspan=2)
#hist(np.log10(N_chi_inp),bins=5,normed=True)
h, b = np.histogram(np.log10(N_chi_inp_error), bins=np.linspace(np.log10(np.amin(N_chi_inp_error)), np.log10(np.amax(N_chi_inp_error)), 100), density=True)
#bar(b[:-1],h,width=(max(h)-min(h))/len(N_chi_inp))
...
h, b = np.histogram(T_chi_inp, bins=np.linspace((np.amin(T_chi_inp)), (np.amax(T_chi_inp)), 100), density=True)
#bar(b[:-1],h,width=(max(h)-min(h))/len(T_chi_inp))
bar(b[:-1],h/np.sum(h),width=0.1)
...
subplot2grid((6,7), (5,0), colspan=4,rowspan=2)
#hist(T_chi_inp,bins=5,normed=True)
h, b = np.histogram(T_chi_inp_error, bins=np.linspace((np.amin(T_chi_inp_error)), (np.amax(T_chi_inp_error)), 100), density=True)
#bar(b[:-1],h,width=(max(h)-min(h))/len(T_chi_inp))
...
h, b = np.histogram(np.log10(N_data_inp), bins=np.linspace(np.log10(np.amin(N_data_inp)), np.log10(np.amax(N_data_inp)), 100), density=True)
...
h, b = np.histogram(T_data_inp, bins=np.linspace((np.amin(T_data_inp)), (np.amax(T_data_inp)), 100), density=True)
...
figure(2)
plot(inp_N, chi_N, 'b.', markersize=2)
xlabel(r'$N_{data}\/(K)$')
ylabel(r'$N_{\chi^{2}}\/(K)$')
#xlim(min(inp_T),max(chi_T))
#ylim(min(inp_T),max(chi_T))
...
xlabel(r'$log_{10}N_{input}\/(cm^{-2})$')
...
figure(5)
plot(np.log10(inp_T), (inp_N/chi_N), 'b.', markersize=2)
xlabel(r'$T_{input}\/(K)$')
...
# Read in all data
blue_raw = np.loadtxt('../../filament/blue/core/image_trans.out',skiprows=6)
green_raw = np.loadtxt('../../filament/green/core/image_trans.out',skiprows=6)
plw_raw = np.loadtxt('../../filament/plw/core/image_trans.out',skiprows=6)
pmw_raw = np.loadtxt('../../filament/pmw/core/image_trans.out',skiprows=6)
psw_raw = np.loadtxt('../../filament/psw/core/image_trans.out',skiprows=6)
red_raw = np.loadtxt('../../filament/red/core/image_trans.out',skiprows=6)
