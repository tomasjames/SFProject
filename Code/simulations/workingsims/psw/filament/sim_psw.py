...
# Check to see whether image.out exists. If so, leave it alone and if not write it to the working directory
if os.path.isfile('image.out') == True:
    print '\nimage.out already exists; no need to write/overwrite\n'
else:
    # This line writes the radmc3d.inp file and then immediately closes it
    open('image.out', 'w').close()
    print '\n--->image.out did not already exists; a blank file called image.out has been written to the working directory\n'

############################## Set up initial model ############################
# Call the data file generation generation script to write the necessary files to the working directory
execfile('/Users/tomasjames/Documents/University/Project/ZiggyStarDust/Code/datafiles/vanilla/datafilegenv3.py')
