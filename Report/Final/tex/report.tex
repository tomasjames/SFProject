...
\usepackage[style=authoryear-icomp, firstinits=true, maxcitenames=3, uniquelist=false, backend=biber]{biblatex}
...
\begin{abstract}
\begin{comment}
  We present an investigation probing whether dust emission in molecular clouds can be used as a surrogate marker of the star forming regions within these clouds. A simple scenario of a spherical, isothermal molecular cloud at $T=10K$ was simulated using RADMC-3D \parencite{RADMC-3D} across the 3 SPIRE \parencite{SPIRE} wavebands centered on 250 $\mu m$, 350 $\mu m$ and 500 $\mu m$ as well as the 3 PACS \parencite{PACS} wavebands centered on 75 $\mu m$, 110 $\mu m$ and 170 $\mu m$. To better simulate an object observed by Herschel, this data was `degraded` by accounting for the filter's transmission curve. Subsequent analysis produced Spectral Energy Distributions (SEDs) on a pixel by pixel basis, where the goodness-of-fit is assessed using a $\chi^{2}$ test. This routine derived the best fit column density $N$ and temperature $T$ at each pixel, subsequently leading to construction of 2-dimensional maps of these regions. Finally this machinery was applied to synthetic Herschel data from the SPH simulation Arepo \parencite{arepo}. The dust properties recovered using the $\chi^{2}$ test are of the correct range however the observed biases stated in \textcite{noise}, \textcite{noiseb} and \textcite{kelly} were evident such that the $\chi^{2}$ test underestimates values of $T$ at lower temperatures and overestimates values of $T$ at higher temperatures, owing to the line of sight temperature variations as stated in \textcite{noise}.
\end{comment}

We present an investigation probing whether dust emission in molecular clouds can be used as a surrogate marker of the star forming regions within these clouds. To simulate a molecular cloud we use synthetic Herschel data from the moving-mesh SPH code Arepo \parencite{arepo}. Computing dust emission intensity via the ray-tracing code RADMC-3D \parencite{RADMC-3D} in each of the 3 SPIRE \parencite{SPIRE} wavebands (centered on 250 $\mu m$, 350 $\mu m$ and 500 $\mu m$) as well as the 3 PACS \parencite{PACS} wavebands (centered on 75 $\mu m$, 110 $\mu m$ and 170 $\mu m$) produces synthetic, composite observations. We `degrade` the images by accounting for the given filter`s transmission curve, in order to better simulate true Herschel data.  These observations are reduced to Spectral Energy Distributions (SEDs) on a pixel-by-pixel basis and a modified blackbody fitted to the points, estimating the best fit $N$ and $T$ at each pixel via a $\chi^{2}$ minimisation test. These parameters, along with the Arepo input parameters, are used to produce 2-dimensional maps of both $N$ and $T$. We also analyse these maps using dendrograms to probe levels of structure, giving another measure of comparison. We compare both maps with each other (as well as their accompanying dendrograms) and determine that the whilst the $\chi^{2}$ minimised maps retain their structure, the observed biases stated in \textcite{noise, noiseb} and \textcite{kelly} are evident such that the $\chi^{2}$ test underestimates values of $T$ at lower temperatures and overestimates values of $T$ at higher temperatures. We observe the same effects as \textcite{noise} in that the single fit $T$ used here is an inaccurate assumption, given the line of sight temperature variations.
\end{abstract}
